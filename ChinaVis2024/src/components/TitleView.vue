...
export default {
    ...
    methods: {
        drawTitles(){
            ...
            const RO = 30, RI = 15, Interval = 60;
            
            const svg = d3.select('.TitlesContainer').append('svg')
                .attr('viewBox', [0, 0, WIDTH, HEIGHT])
                .attr('width', WIDTH)
                .attr('height', HEIGHT)
            const R_OutScale = d3.scaleLinear().domain([0, 1]).range([RI, RO]);
            const R_InScale = d3.scaleLinear().domain([0, 1]).range([RI/3, RI]);
            const IndexOfSub = (belong) => this.dataobj.sub_knowledges.indexOf(belong)
