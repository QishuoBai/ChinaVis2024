...
export default {
    ...
    methods:{
        draw_Stu(){
            const svg = d3.select(this.$refs.Rose_container).append('svg')
                .attr('viewBox', [0, 0, this.layout.HEIGHT*this.layout.WeeksNum, this.layout.HEIGHT])
            ...
            this.dataobj.roseArr.forEach((obj,index) => {
                ...
                rose_group.append('g').attr('class','rose')
                    .attr("transform", ()=>`translate(${index*this.layout.HEIGHT + this.layout.HEIGHT/2}, ${this.layout.HEIGHT/2})`)
                    .selectAll('path')
                    .data(obj.rose)
                    .enter()
                    .append('path')
